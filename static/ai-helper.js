...
const AI = (() => {
  ...
  const W = {
    ...
    BREAK:     [ 0.1, -0.8, 1.6,  0.2, -0.1, -2.5, -0.4,  1.2,  0.1 ],
  ...
  function features(ctx) {
    ...
    return [
      1, stopMin, contDriveMin,
      hourSin, hourCos, avgSpeedKmh,
      lastIsDrive, lastIsBreak, lastIsWork
    ];
  }
...
function maybeSuggestSwitch() {
  ...
  // не предлагать слишком часто (1 раз в 2 минуты) и если режим совпадает
  if (top >= SUGGESTION_THRESHOLD &&
      pred.label !== window.current?.type &&
      (now - lastSuggestionAt > 120000)) {
